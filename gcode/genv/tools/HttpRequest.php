...
class HttpRequest
{
     ...
     * 上传图片请求事例:
        ...
        \Genv\tools\HttpRequest::httpPost($url,$data)
     */
    ...
    static public function httpPost($url, $param = array(), $httpHeaders = array(),$proxy='', $http_code = 200)
    {
        ...
        if (intval($info["http_code"]) === 0) {
            throw new \Exception('[\Genv\tools\HttpRequest::httpPost]: POST request was aborted ! Request url :' . $url . ' , post request data : ' . var_export($param));
        }elseif(intval($info["http_code"]) != $http_code){
            throw new \Exception('[\Genv\tools\HttpRequest::httpPost]: POST request was aborted ! Request url :' . $url . ' , post request data : ' . var_export($param).' ,return code : '.$info["http_code"] .' ,return content : '.$content);
        } else {
            return $content;
        }
    }
    ...
    static public function httpGet($url, $param = array(), $httpHeaders = array(),$proxy= '',  $http_code = 200)
    {
        ...
        /** 验证网络请求状态 */
        if (intval($info["http_code"]) === 0) {
            throw new \Exception('[\Genv\tools\HttpRequest::httpGet]: GET request was aborted ! Request url :' . $url );
        }elseif(intval($info["http_code"]) != $http_code){
            throw new \Exception('[\Genv\tools\HttpRequest::httpGet]: GET request was aborted ! Request url :' . $url .' ,return code : '.$info["http_code"] .' ,return content : '.$content);
        } else {
            return $content;
        }
    }
